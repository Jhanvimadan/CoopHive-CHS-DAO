<script>
  ...
  const { userId, token } = get(authStore); // Retrieve userId and token from the store
  ...
  try {
    ...
    if (response.ok) {
      ...
      showToast(`Proposal listed successfully! Redirecting to dashboard`, 'success');
      setTimeout(() => {
      ...
      const errorMessage = errorData.message || 'Failed to display the proposal. Try again.';
    ...
    showToast(`An error occurred while listing the proposal: ${errorMessage}`, 'error');
    return null;
  }
};
</script>
...
>
  <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
    <h2 class="mb-4 text-2xl font-bold text-gray-900 dark:text-white">
      Add a New Proposal to your Collection 🚀
    </h2>
      ...
      <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
        <div class="sm:col-span-2">
          ...
          <input
            ...
            placeholder="Enter the proposal name (e.g., Vintage Car)"
            required
          />
        </div>
        <div class="sm:col-span-2">
